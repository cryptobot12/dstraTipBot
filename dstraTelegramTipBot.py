...
def info(bot, update):
    ...
    use them like so:
     ...
     WHERE:
    ` 
       [address] = withdraw #DSTRA address
          [user] = telegram username 
        [amount] = amount of #dstra to utilise 
    `

     *NOTE*:
      - don't deposit a significant amount of #DSTRA through this #BOT
      - make sure that you enter a valid #DSTRA address when you perform a withdraw
...
def tip(bot, update):
    ...
    if amount > 100000:
        msg = "Please send a lower amount (max: 100,000 DSTRA)!"
        send_message(bot, chat_uuid, msg, False)
        return False
    if target == user_name:
...
def withdraw(bot, update):
    ...
    if not isValidAddress(message[1]):
        msg = 'Please input a valid DSTRA address!'
        send_message(bot, chat_uuid, msg, False)
        return False
    if not isValidAmount(message[2]):
        msg = 'Please input a valid amount (ex: 1000)!'
...
def price(bot, update):
    ...
    getmarket = 'https://chart.meanxtrade.com/info.php?market=DSTRA_LTC'
